...
import static org.ballerinalang.plugins.idea.psi.impl.BallerinaPsiImplUtil.getContextElement;

public class BallerinaReference extends BallerinaReferenceBase<BallerinaReferenceExpressionBase> {
    ...
    private static final ResolveCache.PolyVariantResolver<BallerinaReference> MY_RESOLVER =
            (r, incompleteCode) -> r.resolveInner();

    public BallerinaReference(@NotNull BallerinaReferenceExpressionBase o) {
...
//                : processUnqualifiedResolve((BallerinaFile) file, processor, state);
    ...
    //  private boolean processQualifierExpression(@NotNull BallerinaFile file,
    //                                             @NotNull BallerinaReferenceBase qualifier,
    ...
    //    }
    //    if (target instanceof PsiDirectory && !processDirectory((PsiDirectory)target, file, null, processor, state,
    // false)) return false;
    ...
    //        if (ref != null && ref.resolve() == ref) return processor.execute(myElement, state); // a bit hacky
    // resolve for: var a C.foo; a.b
    //      }
    //    }
    //    return true;
    //  }

    //  private boolean processGoType(@NotNull GoType type, @NotNull GoScopeProcessor processor, @NotNull
    // ResolveState state) {
    ...
    //  private boolean processPointer(@NotNull GoPointerType type, @NotNull GoScopeProcessor processor, @NotNull
    // ResolveState state) {
    //    GoType pointer = type.getType();
    //    return pointer == null || processExistingType(pointer, processor, state) && processTypeRef(pointer,
    // processor, state);
    //  }

    //  private boolean processTypeRef(@Nullable GoType type, @NotNull GoScopeProcessor processor, @NotNull
    // ResolveState state) {
    //    if (type == null) {
    //      return true;
    //    }
    ...
    //  private boolean processExistingType(@NotNull GoType type, @NotNull GoScopeProcessor processor, @NotNull
    // ResolveState state) {
    ...
    //    if (!(myFile instanceof GoFile) || !allowed(file, myFile, ModuleUtilCore.findModuleForPsiElement(myFile)))
    // return true;
    //
    ...
    //    if (canProcessMethods && parent != null && !processNamedElements(processor, state, parent.getMethods(),
    // localResolve, true)) return false;
    //
    ...
    //          if (!processNamedElements(processor, state, ContainerUtil.createMaybeSingletonList(anon),
    // localResolve)) return false;
    //        }
    //      }
    ...
    //      if (!processNamedElements(processor, state, ((GoInterfaceType)type).getMethods(), localResolve, true))
    // return false;
    ...
    //  private boolean processInTypeRef(@Nullable GoTypeReferenceExpression e, @NotNull GoScopeProcessor processor,
    // @NotNull ResolveState state) {
    ...
    //        if (inner != null && !processGoType(inner, processor, state.put(DONT_PROCESS_METHODS, true))) return
    // false;
    //      }
    //      return true;
    //    }
    ...
    //    if (grandPa instanceof GoSelectorExpr && !processSelector((GoSelectorExpr)grandPa, processor, state,
    // parent)) return false;
    //
    ...
    //    if (!processDirectory(file.getOriginalFile().getParent(), file, file.getPackageName(), processor, state,
    // true)) return false;
    //    return processBuiltin(processor, state, myElement);
    //  }

    //  private boolean processReceiver(@NotNull GoScopeProcessor processor, @NotNull ResolveState state, boolean
    // localResolve) {
    //    GoScopeProcessorBase delegate = createDelegate(processor);
    ...
    //  private boolean processBlock(@NotNull GoScopeProcessor processor, @NotNull ResolveState state, boolean
    // localResolve) {
    //    GoScopeProcessorBase delegate = createDelegate(processor);
    ...
    //      List<GoReferenceExpression> refs = ContainerUtil.newArrayList(PsiTreeUtil.findChildrenOfType(e,
    ...
    @Override
    protected boolean processFileEntities(@NotNull BallerinaFile file,
                                          @NotNull BallerinaScopeProcessor processor,
                                          @NotNull ResolveState state,
                                          boolean localProcessing) {
        //        if (!processNamedElements(processor, state, file.getConstants(), o -> !Comparing.equal(GoConstants
        // .IOTA, o
        //     .getName()) ||
        //                                                                        !builtin(o) ||
        ...
        //        Condition<GoNamedElement> dontProcessInit = o -> o instanceof GoFunctionDeclaration && !Comparing
        // .equal(o.getName(), GoConstants.INIT);
        //        if (!processNamedElements(processor, state, file.getFunctions(), dontProcessInit, localProcessing,
        // false)) return false;
    ...
    public PsiElement handleElementRename(@NotNull String newElementName) throws IncorrectOperationException {
        getIdentifier().replace(BallerinaElementFactory.createIdentifierFromText(myElement.getProject(),
                newElementName));
        return myElement;
    }
    ...
    public boolean equals(Object o) {
        return this == o || o instanceof BallerinaReference && getElement() == ((BallerinaReference) o).getElement();
    }
