...
package org.ballerinalang.plugins.idea.stubs.filter;
...
public class BallerinaIdFilter extends IdFilter {
    public static final Logger LOG = Logger.getInstance("#com.intellij.ide.util.gotoByName" +
    ...
    private BallerinaIdFilter(@NotNull BitSet idSet) {
        myIdSet = idSet;
    }
    ...
    public static IdFilter getProductionFilter(@NotNull Project project) {
        return createIdFilter(project, PRODUCTION_FILTER, file -> !file.isDirectory() /*&& !GoTestFinder.isTestFile
        (file)*/);
    }
    ...
    private static IdFilter createIdFilter(@NotNull Project project,
        ...
        return CachedValuesManager.getManager(project).getCachedValue(project, cacheKey, () -> {
            ...
            return CachedValueProvider.Result.create(new BallerinaIdFilter(bitSet), ProjectRootManager.getInstance
                            (project),
    ...
    public static IdFilter getFilesFilter(@NotNull GlobalSearchScope scope) {
        if (scope instanceof GlobalSearchScope.FilesScope) {
            ...
            return new BallerinaIdFilter(bitSet);
        }
        return null;
    }
